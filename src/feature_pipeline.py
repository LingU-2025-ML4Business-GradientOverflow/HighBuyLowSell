...
def safe_divide(numerator, denominator, fill_value=0.0, epsilon=1e-8):
    ...
    result = np.where(
        np.abs(denominator) < epsilon, fill_value, numerator / (denominator + epsilon)
    )
    return result
...
class MacroeconomicFeaturesTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        if self.auto_download:
            print(
                "Downloading macroeconomic features (S&P500, NASDAQ, Interest Rate, VIX)..."
            )
            try:
...
class AdvancedCompanySpecificTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        if nvda_m.any():
            ...
            df.loc[nvda_m, "nvda_supply_momentum"] = safe_divide(
                df.loc[nvda_m, "return_5d"], df.loc[nvda_m, "volatility_5d"]
            )
            df.loc[nvda_m, "nvda_price_overheat"] = safe_divide(
                df.loc[nvda_m, "close"], df.loc[nvda_m, "close"].rolling(20).mean()
            )

        msft_m = df["symbol"] == "MSFT"
        if msft_m.any():
            df.loc[msft_m, "msft_efficiency_ratio"] = safe_divide(
                df.loc[msft_m, "return_5d"],
                df.loc[msft_m, "return_1d"].abs().rolling(5).sum(),
            )
        ...
        if googl_m.any():
            ...
            df.loc[googl_m, "googl_mean_reversion"] = safe_divide(
                ...
                df.loc[googl_m, "close"].rolling(20).std(),
            )
            df.loc[googl_m, "googl_liquidity_density"] = safe_divide(
                df.loc[googl_m, "volume"],
                df.loc[googl_m, "high"] - df.loc[googl_m, "low"],
            )
        ...
        if hk_m.any():
            ...
            df.loc[hk_m, "tencent_capital_inflow"] = safe_divide(
                df.loc[hk_m, "volume"], df.loc[hk_m, "volume"].rolling(20).mean()
            )
            df.loc[hk_m, "tencent_gap_dynamic"] = safe_divide(
                df.loc[hk_m, "open"] - df.loc[hk_m, "close"].shift(1),
                df.loc[hk_m, "close"].shift(1),
            )

        baba_m = df["symbol"] == "BABA"
        if baba_m.any():
            df.loc[baba_m, "baba_retail_inflection"] = df.loc[baba_m, "ma_gap_5"].diff()
            df.loc[baba_m, "baba_vol_clustering"] = safe_divide(
                df.loc[baba_m, "volatility_5d"],
                df.loc[baba_m, "volatility_5d"].rolling(20).mean(),
            )
        ...
        if bidu_m.any():
            ...
            df.loc[bidu_m, "bidu_cash_stability"] = safe_divide(
                df.loc[bidu_m, "close"].rolling(10).median(), df.loc[bidu_m, "close"]
            )
            df.loc[bidu_m, "bidu_rebound_force"] = safe_divide(
                df.loc[bidu_m, "low"].rolling(20).min(), df.loc[bidu_m, "close"]
            )
